...
| Modelo STEP de fabricante (LCSC, EasyEDA) pode vir 180° girado em relação ao footprint, e o 2D aceita nos dois sentidos | confira contra o `F.Fab` do footprint do KiCad, que diz onde o corpo tem de estar, e corrija pela tabela `footprints.MODELO_GIRADO`; a regra `ME4` do `dry_run_pcb.py` mede isso a cada rodada (as ilhas têm de ficar sob o corpo) |
