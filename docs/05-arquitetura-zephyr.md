...
| `usb` | `src/svc/usb/usb_svc.c` (só no alvo com pilha USB) | 8 | 2048 B | caixa de entrada; tique de 1 s | liga o USB quando o cabo entra: porta serial com os comandos do legacy e, no modo USB, o disco do ciclista no PC; a interrupção do CDC só enfileira bytes |
...
| `usb` | `usb_thread` 72 + `cdc_process` 56 + `cmd_parser_feed` 24 + `app_cmd_handle` 112 + publicação no zbus (~0,35 KB) + log (~0,3 KB); o caminho de ligar o barramento é mais raso (`usb_go` 56 + as funções do stack, todas abaixo de 120 B) | ~0,9 KB | 2048 B | ~1,1 KB |
