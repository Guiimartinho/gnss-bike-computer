...
- **Compila** no NCS v3.3.0 (FLASH 294.796 B, RAM 118.080 B, 7 avisos conhecidos) e **passa em 6 conjuntos de testes de host** (42 casos). **Nada foi testado na placa** nem no nRF52840-DK.
...
|---|---|---|---|---|
| Execução | task manager cooperativo, eventos, tick de 5 ms | 3 threads preemptivas por polling (`main_loop` 100 ms, `display` 250 ms, `sensor` 1 s) | diferente | GPS saiu da ISR nesta revisão; o modelo ainda não tem trava ([05](05-arquitetura-zephyr.md)) |
...
| alto | modelo | `current_att`, `stats`, `gps_data` e outros são escritos numa thread e lidos pela `display` sem trava |
