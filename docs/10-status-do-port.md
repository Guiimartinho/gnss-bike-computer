...
- **Compila** no NCS v3.3.0 sem aviso (nRF52840 DK: FLASH 473.436 B, RAM 220.352 B; nRF54LM20 DK: FLASH 487.172 B, RAM 249.456 B) e **passa em 12 conjuntos de testes de host** (115 casos). **Nada foi testado na placa** nem nos DKs.
...
| Energia | latch pelo IO0 do STC3100, auto-off 15 min | máquina de sistema no SMF (`sys_fsm.c`): auto-off de 15 min com os pings do legacy (posição em CRS, PRC e DBG; rolo em FEC), desligamento em etapas com a resposta de cada serviço, ship mode do nPM1300 ou System OFF; medidor MAX17262 por driver próprio, com bateria fraca (10 %) e crítica (0 %) | parcial | faltam o carregador (nPM1300) e o painel (AEM10900); `test_sys_fsm`, `test_max17262`, `test_battery`; não testado na placa |
...
| GPS e NMEA | TinyGPS++ com checksum, PMTK010→PMTK251, WDT de baud | API de GNSS do Zephyr pelo alias `gnss` (`gnss-nmea-generic` nos dois DKs), uma posição por época | parcial | o driver UBX do M10, com LEAP e backup, vem no passo do GNSS; sem EPO nem AssistNow |
...
| Log no SD | `@DDMMYY.txt`, 19 campos | FatFs montado pelo serviço de armazenamento; CSV de 13 campos | parcial | o formato do legacy é a fase 3; não testado com cartão |
...
| GNSS | API de GNSS do Zephyr; os callbacks rodam num workqueue próprio do modem | build |
