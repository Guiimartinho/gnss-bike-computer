...
| `main_loop` (`main_thread`) | 5 | 4096 B | 100 ms | GPS, modelo, Kalman, botões, log |
| `sensor` | 6 | 1024 B | 100 ms (trabalho a cada 1 s) | leitura de sensores, bateria no BLE |
| `display` | 7 | 2048 B | 50 ms (quadro a cada 250 ms) | desenhar e enviar o LCD |
...
2. **Um dono por estrutura.** O modelo (`attitude`, `boucle`, `segment`, `gps_data`) é escrito pela `main_loop`. Quem lê de outra thread precisa de `k_mutex` curto ou de uma cópia publicada com contador de sequência. Hoje a `display` lê sem trava: é um defeito aberto (fase 1 do roteiro).
